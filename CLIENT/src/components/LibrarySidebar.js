...
const LibrarySidebar = ({ watchlists, username, sidebarOpen, toggleSidebar }) => {
  ...
  return (
    <div className="relative z-40">
      <div className={`rounded-lg fixed top-24 left-0 h-[calc(100vh-6rem)] bg-[#232323] p-4 transition-transform duration-300 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`} style={{ width: '240px' }}>
        ...
        <ul>
          {watchlists?.map(list => (
            <li key={list.id} className="cursor-pointer mb-2 flex flex-col items-start text-[#f6f6f6]" onClick={() => navigate(`/list/${username}/${encodeURIComponent(list.name)}/${list.id}`)}>
